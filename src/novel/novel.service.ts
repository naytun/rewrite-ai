	...
	async readChapter(volume: string, chapter: string): Promise<any> {
		try {
			...
			const result = await askAI({
				question: `Rewrite following contents to be in shorter sentences and be concise. Keep dialogue, just rephrase for better reading. Create paragraphs as needed. Use only common words for better readability . ---\n ${plainText}`,
			})
